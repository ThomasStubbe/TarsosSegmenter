...
import be.tarsos.tarsossegmenter.model.segmentation.Segmentation.IndexReference;
...
public class SegmentationPart implements Comparable<SegmentationPart> {
    ...
    public SegmentationPart(float begin, float end) {
        this.begin = be.tarsos.tarsossegmenter.util.math.Math.round(begin, 2);
        this.end = be.tarsos.tarsossegmenter.util.math.Math.round(end, 2);
        label = EMPTY_LABEL;
        color = Color.WHITE;
    }
...
//        }
    ...
    public void split(float time) {
        time = be.tarsos.tarsossegmenter.util.math.Math.round((float) time, 2);
