...
public class Segmentation {
    ...
    private IndexReference macroSegmentationIndex; //Determinates the chosen segmentation
    private IndexReference mesoSegmentationIndex;
    private ArrayList<SegmentationList> segmentationSuggestions; //for every suggestion it contains the different labels and a link to a collection containing those segmentationParts    
    ...
    public Segmentation() {
        ...
        segmentationSuggestions.add(new SegmentationList(null, AASModel.MACRO_LEVEL));
        ...
        switch (segmentationLevel) {
            ...
            case AASModel.MESO_LEVEL:
                ...
                for (int i = 0; i < getSegmentation().size(); i++) {
                    size = Math.max(size, getSegmentation().get(i).getSubSegmentationSuggestions().size());
                }
                return size;
            case AASModel.MICRO_LEVEL:
                size = 0;
                for (int i = 0; i < getSegmentation().size(); i++) {
                    for (int j = 0; j < getSegmentation().get(i).getSubSegmentation().size(); j++) {
                        size = Math.max(size, getSegmentation().get(i).getSubSegmentation().get(j).getSubSegmentationSuggestions().size());
                    }
                }
                return size;
        }
    ...
    public ArrayList<SegmentationList> getSegmentationLists(int segmentationLevel) {
        switch (segmentationLevel) {
            case (AASModel.MACRO_LEVEL):
    ...
    public SegmentationPart searchSegmentationPart(double time) {
        ...
        if (!isEmpty()) {
            ...
            if (activeSegmentationLevel == AASModel.MESO_LEVEL || activeSegmentationLevel == AASModel.MICRO_LEVEL) { //meso of micro
    ...
    public SegmentationPart searchSegmentationPart(double time, int level) {
        ...
        if (!isEmpty()) {
            sp = searchSegmentationPartInList(getSegmentation(), time);
            if (level == AASModel.MESO_LEVEL || level == AASModel.MICRO_LEVEL) { //meso of micro
    ...
    public SegmentationPart searchSegmentationPartInList(SegmentationList searchList, double time) {
        int i = 0;
        while (i < searchList.size() && !(searchList.get(i).getBegin() <= time && searchList.get(i).getEnd() > time)) {
            i++;
        }
    ...
    public int getActiveMicroSize() {
        ...
        for (int i = 0; i < getSegmentation().size(); i++) {
            for (int j = 0; j < getSegmentation().get(i).getSubSegmentation().size(); j++) {
                size += getSegmentation().get(i).getSubSegmentation().get(j).getSubSegmentation().size();
            }
        }
        return size;
    }
    ...
    private ArrayList<SegmentationList> getMesoSegmentationLists() {
        ...
        for (int i = 0; i < getSegmentation().size(); i++) {
            if (getSegmentation().get(i).hasSubSegmentation()) {
                activeSegmentationLists.add(getSegmentation().get(i).getSubSegmentation());
            }
        }
        return activeSegmentationLists;
    }
    ...
    private ArrayList<SegmentationList> getMicroSegmentationLists() {
        ...
        for (int i = 0; i < getSegmentation().size(); i++) {
            if (getSegmentation().get(i).hasSubSegmentation()) {
                for (int j = 0; j < getSegmentation().get(i).getSubSegmentation().size(); j++) {
                    if (getSegmentation().get(i).getSubSegmentation().hasSubSegmentation()) {
                        activeSegmentationLists.add(getSegmentation().get(i).getSubSegmentation().get(j).getSubSegmentation());
                    }
                }
            }
        }
    ...
    public void addSegmentationPart(SegmentationPart sp, int segmentationLevel) {
        switch (segmentationLevel) {
            ...
            case AASModel.MESO_LEVEL:
                if (!this.isEmpty()) {
                    if (getSegmentation().getBegin() <= sp.getBegin() && getSegmentation().getEnd() >= sp.getEnd()) {
                        SegmentationPart temp = searchSegmentationPart(sp.getBegin(), AASModel.MACRO_LEVEL);
                        if (temp != null) {
            ...
            case AASModel.MICRO_LEVEL:
                if (!this.isEmpty()) {
                    if (getSegmentation().getBegin() <= sp.getBegin() && getSegmentation().getEnd() >= sp.getEnd()) {
                        SegmentationPart temp = searchSegmentationPart(sp.getBegin(), AASModel.MESO_LEVEL);
                        if (temp != null) {
                            temp.createSubSegmentationSuggestionList();
                            temp.getSubSegmentation().add(sp);
                        } else {
    ...
    public void addSegmentationPoint(float time, int segmentationLevel) {
        ...
        if (Configuration.getBoolean(ConfKey.enable_micro)) {
            while (i < microSegmentationPoints.size() && microSegmentationPoints.get(i) < time) {
                i++;
            }
            if (i == microSegmentationPoints.size()) {
                if (microSegmentationPoints.size() > 0) {
                    if (time - microSegmentationPoints.get(microSegmentationPoints.size() - 1) > 0.5f) {
                ...
                if (microSegmentationPoints.get(i) != time) {
                    if (microSegmentationPoints.get(i) - time > 0.5f && time - microSegmentationPoints.get(i - 1) > 0.5f) {
                        microSegmentationPoints.add(i, time);
                    }
                }
            }
        }
        if (segmentationLevel < AASModel.MICRO_LEVEL && Configuration.getBoolean(ConfKey.enable_meso)) {
            i = 0;
            while (i < mesoSegmentationPoints.size() && mesoSegmentationPoints.get(i) < time) {
                i++;
            }
            if (i == mesoSegmentationPoints.size()) {
                if (mesoSegmentationPoints.size() > 0) {
                    if (time - mesoSegmentationPoints.get(mesoSegmentationPoints.size() - 1) > 0.7f) {
                ...
                if (mesoSegmentationPoints.get(i) != time) {
                    if (mesoSegmentationPoints.get(i) - time > 0.7f && time - mesoSegmentationPoints.get(i - 1) > 0.7f) {
                        mesoSegmentationPoints.add(i, time);
                    }
                }
            }
        }
        if (segmentationLevel == AASModel.MACRO_LEVEL && Configuration.getBoolean(ConfKey.enable_macro)) {
            i = 0;
            while (i < macroSegmentationPoints.size() && macroSegmentationPoints.get(i) < time) {
                i++;
            }
            if (i == macroSegmentationPoints.size()) {
                if (macroSegmentationPoints.size() > 0) {
                    if (time - macroSegmentationPoints.get(macroSegmentationPoints.size() - 1) > 0.8f) {
                ...
                if (macroSegmentationPoints.get(i) != time) {
                    if (macroSegmentationPoints.get(i) - time > 0.8 && time - macroSegmentationPoints.get(i - 1) > 0.8f) {
