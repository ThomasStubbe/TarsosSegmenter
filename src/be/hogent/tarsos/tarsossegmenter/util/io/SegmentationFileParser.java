...
public class SegmentationFileParser {
    ...
    static public void parseFile(String file, Segmentation segmentation) {
        ...
        try {
            ...
            while ((strLine = in.readLine()) != null) { // lijn per lijn inlezen
                ...
                } else if (segmentationLevel!= -1 && strLine.contains("intervals [")) {
                    float begin = Float.valueOf(in.readLine().trim().split(" ")[2]); //begin
                    float end = Float.valueOf(in.readLine().trim().split(" ")[2]);
            ...
            while (!mesoParts.isEmpty()) {
                segmentation.addSegmentationPart(mesoParts.pop(), AASModel.MESO_LEVEL);
            }
            while (!microParts.isEmpty()) {
                segmentation.addSegmentationPart(microParts.pop(), AASModel.MICRO_LEVEL);
            }
        } catch (IOException e) {
            System.out.println("No segmentationfile was found or error while parsing");
        }
    ...
    static public void writeToFile(String file, Segmentation segmentation) {
        ...
        try {
            ...
            ArrayList<SegmentationList> macroSegmentation = segmentation.getSegmentationLists(AASModel.MACRO_LEVEL);
            ArrayList<SegmentationList> mesoSegmentation = segmentation.getSegmentationLists(AASModel.MESO_LEVEL);
            ArrayList<SegmentationList> microSegmentation = segmentation.getSegmentationLists(AASModel.MICRO_LEVEL);
            ...
            if (microSegmentation != null && !microSegmentation.isEmpty()) {
                ...
                out.write("\t\txmax = " + microSegmentation.get(microSegmentation.size() - 1).getEnd() + " \n");
                out.write("\t\tintervals: size = " + segmentation.getActiveMicroSize() + " \n");
                int count = 0;
                for (int i = 0; i < microSegmentation.size(); i++) {
                    for (int j = 0; j < microSegmentation.get(i).size(); j++) {
                        ...
                        if (sp.getComment() != null && !sp.getComment().isEmpty()) {
            ...
            if (mesoSegmentation != null && !mesoSegmentation.isEmpty()) {
                ...
                out.write("\t\txmax = " + mesoSegmentation.get(mesoSegmentation.size() - 1).getEnd() + " \n");
                out.write("\t\tintervals: size = " + segmentation.getActiveMesoSize() + " \n");
                int count = 0;
            ...
            if (macroSegmentation != null && !macroSegmentation.isEmpty()) {
                ...
                out.write("\t\txmax = " + macroSegmentation.get(macroSegmentation.size() - 1).getEnd() + " \n");
                out.write("\t\tintervals: size = " + segmentation.getActiveMacroSize() + " \n");
                int count = 0;
    ...
    static public void parseCSVFile(String file, Segmentation segmentation) {
        ...
        try {
            ...
            while (strLine != null && !(strLine.equals("MACRO") || strLine.equals("MESO") || strLine.equals("MICRO"))) {
                strLine = in.readLine();
                if (strLine != null) {
            ...
            while ((strLine = in.readLine()) != null) { // lijn per lijn inlezen
                ...
                switch (strLine) {
                    ...
                    default:
                        ...
                        SegmentationPart sp = new SegmentationPart(Float.valueOf(data[0]), Float.valueOf(data[1]));
                ...
                if (segmentationLists != null && !segmentationLists.isEmpty()) {
                    ...
                    for (int j = 0; j < segmentationLists.size(); j++) {
                        for (int k = 0; k < segmentationLists.get(j).size(); k++) {
                            SegmentationPart sp = segmentationLists.get(j).get(k);
                            if (sp.getComment() == null) {
                                out.write(sp.getBegin() + ";" + sp.getEnd() + ";" + sp.getLabel() + "\n");
                            } else {
                                out.write(sp.getBegin() + ";" + sp.getEnd() + ";" + sp.getLabel() + ";" + sp.getComment() + "\n");
                            }
                        }
                    }
                }
